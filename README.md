...
| Field | Value | Example |
|---|---|---|
| App Name | `{project-name}` | `payment-service` |
| App Version | `{branch-name}` | `main`, `feature-login-fix` |

This mirrors your Git branch structure directly in SSC — one application per repo, one version per branch. So `payment-service` in SSC will have versions `main`, `develop`, `feature-login-fix` etc., exactly matching your Git branches.
